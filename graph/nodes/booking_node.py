...
- name
- phone
- details: reason for the visit, service, or doctor (e.g., 'تحليل دم', 'كشف عام', 'د. أحمد').
- date
...
"""
    ...
    if parsed.ready_to_save and parsed.confirmed and all_fields_present:
        try:
            ...
            booking_saved     = True
            booking_reference = result.reference_id
            booking_pdf       = result.pdf_bytes
    ...
    return {
        ...
        "booking_reference": booking_reference,   # ← new
        "booking_pdf":       booking_pdf,          # ← new (bytes)
